...
int PlayerDealerPoints(string**, int, int, bool);
void outputfirstlines(string**, bool);
...
char bustcheck (int, bool);
string outputresult(int, int);
bool doublestand(int, int);
void outputlastlines(string**, string*, bool);
bool dealerchoice(int);
void ifTie(string**, string*, int&, int&, int&, char&, char&, char&, char&);
...
void HitorStand(string** hands, string* deck, int& dealCounter, int& dhc, int& phc)
{
  ...
  for(int i = 1; i < 5; i++)
  {
    ...
    //added points funciton into the randomCouts function
    randomCouts(hands, dhc, phc, i, false);
    phist = panswer;
    cin >> panswer;

    //checks if the player decides to first stand then hit, if it it true, then the card will be printed
    //in a previous slot.
    ...
    //if the player's points are greater than 21 then exit loop and print out
    if(PlayerDealerPoints(hands, dhc, phc, false) > 21)
    {
      finaloutput = outputresult(PlayerDealerPoints(hands, dhc, phc, false), PlayerDealerPoints(hands, dhc, phc, true));
      lines[i+1] = printDealerCards(hands, panswer, danswer, dhc-1, phc-1);
      break;
    }
    //checks to see if both the Dealer and Player stand consecutively then prints an outcome
    if(panswer == 's' && danswer == 's')
      if(doublestand(PlayerDealerPoints(hands, dhc, phc, false), PlayerDealerPoints(hands, dhc, phc, true)) == true)
      {
        outputlastlines(hands, lines, false);
        i = 0;
        for(int z = 2; z < 10; z++)
          lines[z] = "   ";
        continue;
    ...
    dhist = danswer;
    sleep(1);
    if (dealerchoice(PlayerDealerPoints(hands, dhc, phc, true)) == true)
    {
      danswer = 'h';
      cout << danswer << endl;
    }
    ...
    if(PlayerDealerPoints(hands, dhc, phc, true) > 21)
    {
      finaloutput = outputresult(PlayerDealerPoints(hands, dhc, phc, false), PlayerDealerPoints(hands, dhc, phc, true));
      break;
    }
    //checks to see if both the Dealer and Player stand consecutively
    if(panswer == 's' && danswer == 's')
    {
      if(doublestand(PlayerDealerPoints(hands, dhc, phc, false), PlayerDealerPoints(hands, dhc, phc, true)) == true)
      {
        ...
        //this funtion is important if the outcome of the game is a tie
        ifTie(hands, deck, dhc, phc, dealCounter, panswer, danswer, phist, dhist);
        continue;
      }
      else
      {
...
string printDealerCards(string** game, char panswer, char danswer, int dhc, int phc)
{
 ...
 void outputfirstlines(string** hands, bool x)
 {
...
void randomCouts(string** hands, int dhc, int phc, int i, bool player)
 ...
 int PlayerDealerPoints(string** hands, int dhc, int phc, bool player)
 {
   //initializes point counters for the player and dealer
   int pp = 0;
   int dp = 0;
   ...
   int aceP = 0;
   int aceD = 0;
   //starts loop to check the second character of each hand
   for(int i = 0; i < phc; i++)
   {
     char first, second;
     second = hands[0][i][1];
        ...
        if(second > '1' && second <= '9')
          ...
          pp += 1;
        else if(second == 'Q' || second == 'K' || second == 'J' || second == '0')
          pp += 10;
 ...
 for(int i = 0; i < dhc; i++)
   ...
   //checks to see if there is an Ace and if it is below 11, then adds 10
   if(aceP > 0)
   {
   if(pp <= 11)
   pp += 10;
...
//takes the scores and returns a string value based on the results
string outputresult(int pp, int dp)
{
  if(!(pp == dp))
  {
  if(pp == 21)
...
void outputlastlines(string** hands, string* lines, bool x)
{
  outputfirstlines(hands, x);
  for(int i = 2; i < 6; i++)
  {
...
//this function is called when both players stand
//returns a bool value if the scores are a tie or not
bool doublestand(int pp, int dp)
{
  if(pp == dp)
    return true;
  else
...
//returns a bool value whether the dealer should hit or not
bool dealerchoice(int dp)
{
  if (dp < 17)
    return true;
  else
...
//function that is called when the outcome is a tie
void ifTie(string** hands, string* deck, int& dhc, int& phc, int& dealCounter, char& panswer, char& danswer, char& phist, char& dhist)
{
...
//resets the hands and the counters
hands = blankHands(hands);
dealCounter = 4;
phc = 2;
dhc = 2;
for(int i = 0; i < 4; i++)
  hands[i%2][i/2] = deck[i];
panswer = 'h'; danswer = 'h'; phist = 'h'; dhist = 'h';
}
