...
void randomCouts(string**, int, int, int);
...
void HitorStand(string** hands, string* deck, int& dealCounter, int& dhc, int& phc)
{
  ...
  for(int i = 1; i < 4; i++)
  {
    ...
    //if(i != 1) lines[i-1] = printDealerCards(hands, panswer, danswer, dhc-1, phc);
    if(i > 1)
    {
    for(int k = 2; k < i+1; k++)
      cout << lines[k] << endl;
    ...
    //testprint(hands, deck, dhc, phc, dealCounter);

    randomCouts(hands, dhc, phc, i);
    phist = panswer;
    cin >> panswer;
...
void addTohands(string** hands, string* deck, int& dealCounter, int& dhc, int& phc, bool player, char answer)
 ...
 void PlayerDealerPoints(string** hands, int dhc, int phc)
 {
   int pp = 0;
   int dp = 0;
   ...
   for(int i = 0; i < phc; i++)
   {
     ...
     second = hands[0][i][1];
        ...
        if(second > '1' && second <= '9')
          ...
          pp += 1;
        else if(second == 'Q' || second == 'K' || second == 'J')
          pp += 10;
        else if (first == '1')
          pp += 10;
   }
 ...
 void outputfirstlines(string** hands)
 {
    ...
    cout << "|  " << hands[0][0] << "  |  " << hands[1][0] << "  |" << endl;
    cout << "|  " << hands[0][1] << "  |  " << hands[1][1] << "  |" << endl;
 }
 ...
 void randomCouts(string** hands, int dhc, int phc, int i)
 {
   PlayerDealerPoints(hands, dhc, phc);
